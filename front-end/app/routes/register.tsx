...
export default function Register() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // On suppose que le backend renvoie 201 ou 200
      navigate("/login");
    } catch (err: any) {
      ...
      <div className="relative grid w-full max-w-6xl grid-cols-1 lg:grid-cols-5 bg-white dark:bg-secondary/30 rounded-2xl shadow-2xl shadow-primary/10 dark:shadow-primary/20 overflow-hidden border border-slate-200 dark:border-slate-800 backdrop-blur-xl">
        <div className="lg:col-span-2 p-8 md:p-12 flex flex-col justify-between order-2 lg:order-1 bg-secondary/5 dark:bg-secondary/20">
          <div>
            ...
            <h1 className="font-display text-slate-900 dark:text-white text-4xl md:text-5xl font-extrabold leading-tight tracking-tighter">
              Démarrez une nouvelle <span className="text-primary">aventure</span>.
            </h1>
            <p className="text-slate-500 dark:text-slate-400 text-base font-normal leading-relaxed mt-4">
              Créez votre profil en quelques instants et débloquez l'accès à des milliers d'opportunités de carrière.
            </p>
          </div>
          <div className="mt-8 lg:mt-0 text-sm">
            <p className="text-slate-500 dark:text-slate-400">
              Vous avez déjà un compte ?
              <Link className="font-medium text-primary hover:underline ml-2" to="/login">
                Se connecter
              </Link>
            </p>
          </div>
        ...
        <div className="lg:col-span-3 p-8 md:p-12 order-1 lg:order-2">
          <form onSubmit={onSubmit} className="flex flex-col gap-6 h-full">
            <h2 className="font-display text-slate-900 dark:text-white text-3xl font-bold mb-4">Rejoignez-nous</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="relative">
                ...
                />
                <label
                  className="absolute left-0 -top-3.5 text-slate-500 dark:text-slate-400 text-sm transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-slate-500 peer-placeholder-shown:top-3 peer-focus:-top-3.5 peer-focus:text-primary peer-focus:text-sm"
                  htmlFor="fullName"
                >
                  Nom complet
                </label>
            ...
            <div className="relative">
              <input
                ...
                className={`peer h-12 w-full border-b-2 ${password !== confirmPassword ? "border-red-500" : "border-slate-300"} bg-transparent text-slate-900 dark:text-white placeholder-transparent focus:outline-none focus:border-primary`}
                id="confirmPassword"
                placeholder="Confirmer le mot de passe"
                type="password"
                required
              />
              <label className={`absolute left-0 -top-3.5 text-sm transition-all ${password !== confirmPassword ? "text-red-500" : "text-slate-500 dark:text-slate-400"} peer-placeholder-shown:text-base peer-placeholder-shown:top-3 peer-focus:-top-3.5 peer-focus:text-sm`} htmlFor="confirmPassword">
                Confirmer le mot de passe
              </label>
              {password !== confirmPassword && <p className="text-red-500 text-xs mt-1">Les mots de passe ne correspondent pas.</p>}
            </div>
